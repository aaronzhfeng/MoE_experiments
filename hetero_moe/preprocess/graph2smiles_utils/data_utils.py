...
def get_graph_from_smiles(smi: str):
    ...
    # Defensive fallback: ensure RxnGraph receives a valid molecule so that
    # downstream accesses to .reac_mol are safe.
    if mol is None:
...
def get_graph_features_from_smi(_args):
    ...
    # Ensure bond_features always has correct shape (N, BOND_FDIM+2) even when empty
    if bond_features:
        b_features = np.array(bond_features, dtype=np.int32)
    else:
        b_features = np.zeros((0, 2 + BOND_FDIM), dtype=np.int32)
    b_features_lens = b_features.shape[0]
    a_graphs = np.array(a_graphs, dtype=np.int32)
    
    # Ensure b_graphs always has correct shape (N, 11) even when empty
    if b_graphs:
        b_graphs = np.array(b_graphs, dtype=np.int32)
    else:
        b_graphs = np.zeros((0, 11), dtype=np.int32)
    ...
    for phase, file_list in fns.items():
        for src_file, tgt_file in file_list:
            for fn in [src_file, tgt_file]:
                ...
                with open(fn, "r") as f:
                    for line in _tqdm(f, total=total_lines, desc=f"vocab:{os.path.basename(fn)}"):
