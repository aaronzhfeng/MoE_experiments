...
import parsing
from data_utils import get_graph_features_from_smi, load_vocab, make_vocab, \
    tokenize_selfies_from_smiles, tokenize_smiles
from train_utils import log_tensor, set_seed, setup_logger
        ...
        for src_file, tgt_file in file_list:
            ...
            for fn, ofn in [(src_file, src_output),
                ...
                with open(fn, "r") as f, open(ofn, "w") as of:
                    ...
                    for i, line in enumerate(_tqdm(f, total=total_lines, desc=f"tokenize:{os.path.basename(ofn)}")):
...
def binarize_s2s(src_file: str, tgt_file: str, prefix: str, output_path: str,
    ...
    seq_features_and_lengths = _tqdm(seq_features_and_lengths, total=len(src_lines), desc="seq_features")
...
def binarize_g2s(src_file: str, tgt_file: str, prefix: str, output_path: str,
    ...
    # Wrap with progress bar
    graph_features_and_lengths = _tqdm(graph_features_and_lengths, total=len(src_lines), desc="graph_features")
...
def preprocess_main(args):
    ...
    if args.do_tokenize:
        ...
        print(f"Tokenization started at: {__import__('datetime').datetime.now()}")
        ...
        print(f"Tokenization finished at: {__import__('datetime').datetime.now()}")

    vocab_file = os.path.join(args.preprocess_output_path,
                              f"vocab_{args.representation_end}.txt")
    if not os.path.exists(vocab_file):
        print("=== STAGE 2: VOCABULARY BUILDING ===")
        print(f"Vocabulary building started at: {__import__('datetime').datetime.now()}")
        make_vocab(
            fns=fns,
        ...
        print(f"Vocabulary building finished at: {__import__('datetime').datetime.now()}")
    else:
        print("=== STAGE 2: VOCABULARY BUILDING ===")
        print("✅ Vocabulary file already exists, skipping vocabulary building")
    ...
    print(f"Feature extraction started at: {__import__('datetime').datetime.now()}")
    ...
    for phase, file_list in fns.items():
        ...
        print(f"✅ {phase} set completed")
    
    print("✅ FEATURE EXTRACTION COMPLETED")
    print(f"Feature extraction finished at: {__import__('datetime').datetime.now()}")
