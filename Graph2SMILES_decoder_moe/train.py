...
from models.graph2seq_series_rel_dense import Graph2SeqSeriesRel as Graph2SeqSeriesRelDense
...
def get_train_parser():
    ...
    parser.add_argument(
        ...
        help="Which GPU index to use (e.g. 1 for cuda:1)"
    )
    parser.add_argument(
        "--type", type=str, default="moe",
        help="Which type of model to use (e.g. moe, dense)"
    )
    return parser
...
def main(args):
    ...
    try:
        ...
        logging.warning("Falling back to CPU (CUDA unsupported for this GPU).")
    ...
    if args.model == "s2s":
        ...
        if args.type == "moe":
            ...
            model_class = Graph2SeqSeriesRelDense
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if args.do_profile:
                logging.info(
                    prof.key_averages(group_by_input_shape=args.record_shapes)
            ...
            if (accum == 0) and (total_step > 0) and (total_step % args.log_iter == 0):
                logging.info(
                    f"Step {total_step}, loss: {np.mean(losses)}, acc: {np.mean(accs)}, "
            ...
            if (accum == 0) and (total_step > 0) and (total_step % args.eval_iter == 0):
                ...
                logging.info(
                    f"Evaluation (with teacher) at step {total_step}, eval loss: {eval_meters[0]}, "
                    f"eval acc: {eval_meters[1]}"
                )
                sys.stdout.flush()
                model.train()

            # evaluation without teacher & save
            if (accum == 0) and (total_step > 0) and (total_step % args.save_iter == 0):
                    ...
                    for eval_idx, eval_batch in enumerate(valid_loader):
                        ...
                        for i, prediction in enumerate(predictions):
                            ...
                            if eval_idx % 20 == 0 and i == 0:
                                logging.info(
                                    f"Target text: {' '.join([vocab_tokens[idx] for idx in tgt_token_ids])}"
                                )
                                logging.info(
                                    f"Predicted text: {' '.join([vocab_tokens[idx] for idx in prediction])}"
                                )
                                logging.info(
                                    f"acc_token: {acc_token}, acc_seq: {acc_seq}\n"
                                )

                logging.info(
                    f"Evaluation (without teacher) at step {total_step}, "
                ...
                torch.save(
                    state,
                    os.path.join(args.save_dir, f"model.{total_step}_{n_iter}.pt")
                )
